...
export default async function Page() {
  ...
  if (!user && !pin) redirect('/stores/plymouth/login')
  const db = createAdminClient()
  const { data: org, error } = await db.from('organizations').select('id').eq('slug', 'plymouth-township-fire-department').eq('is_active', true).maybeSingle()
