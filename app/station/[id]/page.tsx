...
export default async function Station({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {data:syncs}=collections.length ? await d.db.from('campaign_shopify_collections').select('last_synced_at').in('campaign_id',collections.map(c=>c.id)).order('last_synced_at',{ascending:false}).limit(1) : {data:[]}
  const lastSynced=syncs?.[0]?.last_synced_at||null
  ...
  return <PortalShell org={d.station} campaign={collections[0]} campaigns={collections} userEmail={d.user.email || ""} organizationId={id} platform={d.admin} lastSynced={lastSynced}><div className="station-app" style={{ '--station-color': color } as React.CSSProperties}>
    ...
    <div style={{marginBottom:20}}><Link href={"/station/"+id+"/collections"}>Manage Shopify collections →</Link></div><section className="station-section-head"><div><h2>Your collections</h2><p>Cash out each collection once its available earnings reach $100. Every request requires approval.</p></div></section>
    <div className="station-grid">{collections.map(c => { const goal = Number(c.goal_amount || 0); return <article className="station-card" key={c.id}><span className="station-eyebrow">{c.status}</span><h2>{c.name}</h2><strong className="station-amount">{money(c.earned)}</strong><p>earned for your station</p>{goal > 0 && <><progress aria-label={c.name + " fundraising goal"} max={goal} value={Math.max(0, Number(c.earned))} /><p>{money(c.earned)} of {money(goal)} goal</p></>}<div className="station-detail"><span>Available <b>{money(c.available)}</b></span><span>Pending <b>{money(c.pending)}</b></span><span>Paid <b>{money(c.paid)}</b></span></div><StationCashOut campaignId={c.id} available={Number(c.available)} canRequest={d.canRequest} open={c.has_open_request} /><Link href={'/station/' + id + '/progress?campaign=' + c.id}>View collection details →</Link></article> })}</div>
