...
export default async function DepartmentPortal({searchParams}:{searchParams:Promise<{org?:string}>}){
  ...
  const [{data:org},{data:campaigns},{data:orders},{data:items},{data:payouts},{data:allOrgs}]=await Promise.all([
    db.from("organizations").select("id,name,organization_type,logo_url,contact_name").eq("id",organizationId).maybeSingle(),
  ...
  return <div className="portal-shell">
    <aside className="portal-sidebar">
      <Link href={"/portal?org="+organizationId} className="portal-brand">
        <span className="portal-mark">FC</span>
        <span><strong>{org.name}</strong><small>Department Portal</small></span>
      </Link>

      {platform&&<div className="portal-preview">Platform preview</div>}

      {platform&&<form>
        <label>Preview department
          <select name="org" defaultValue={organizationId}>
            {(allOrgs||[]).map((o:any)=><option value={o.id} key={o.id}>{o.name}</option>)}
          </select>
        </label>
        <button className="portal-mini">Open</button>
      </form>}
    ...
    <main className="portal-main">
      <header className="portal-head">
        <div>
          <div className="portal-kicker">DEPARTMENT OVERVIEW</div>
          <h1>{org.name}</h1>
          <p>See fundraiser performance, orders, payouts and member access in one place.</p>
        </div>
      ...
      <section className="portal-grid" id="campaigns">
        <article className="portal-card">
          ...
          {(campaigns||[]).map((c:any)=><div className="portal-row" key={c.id}>
            <div><strong>{c.name}</strong><small>{c.status}</small></div>
            <span>{money(Number(c.goal_amount||0))} goal</span>
          </div>)}
        ...
        <article className="portal-card" id="payouts">
          ...
          {(payouts||[]).slice(0,5).map((p:any)=><div className="portal-row" key={p.id}>
            <div><strong>{money(Number(p.payout_amount||0))}</strong><small>{new Date(p.created_at).toLocaleDateString()}</small></div>
            <span className={"portal-pill "+p.status}>{p.status}</span>
          </div>)}
      ...
      <section className="portal-card" id="orders">
        ...
        <div className="portal-table-wrap">
          <table className="portal-table">
            ...
            <tbody>
              {(orders||[]).slice(0,12).map((o:any)=><tr key={o.id}>
                <td>#{o.shopify_order_number||"—"}</td>
                <td><span className={"portal-pill "+o.status}>{o.status}</span></td>
