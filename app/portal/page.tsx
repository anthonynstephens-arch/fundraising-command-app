...
export default async function PortalOverview({searchParams}:{searchParams:Promise<{org?:string;campaign?:string}>}){
  ...
  return <PortalShell org={d.org} campaign={d.campaign} campaigns={d.campaigns} userEmail={d.user.email||""} organizationId={d.organizationId} platform={d.platform} lastSynced={d.lastWebhook?.created_at||null}>
    ...
    <section className="agency-kpis">
      ...
      <div><span>NET ELIGIBLE SALES</span><strong className="blue">{money(s.eligible)}</strong><small>after item refunds</small></div>
      <div><span>EST. FUNDRAISING PROCEEDS</span><strong className="orange">{money(s.raised)}</strong><small>snapshotted contribution rules</small></div>
    ...
    <section className="agency-grid-2">
      <article className="agency-card">
        ...
        <div className="agency-detail-list">
          ...
          <div><span>Store status</span><b className="agency-pill active">{d.campaign?.status||"—"}</b></div>
          <div><span>Storefront</span><b>{d.campaign?.public_store_url?"Connected":"Not set"}</b></div>
        </div>
      </article>
      <article className="agency-card">
        <header><div><h2>Top Products</h2><p>Best-selling items this campaign</p></div><span>Based on gross sales</span></header>
        <div className="agency-product-ranks">{top.length?top.map((p,i)=><div key={p.title}><div className="agency-prod-thumb">{i+1}</div><div><strong>{i+1}. {p.title}</strong><small>{p.qty} sold · <b className="green">{money(p.raised)} raised</b></small><div className="agency-bar"><i style={{width:(top[0]?Math.max(8,(p.gross/top[0].gross)*100):0)+"%"}}/></div></div><b>{money(p.gross)}</b></div>):<div className="agency-empty">No product sales yet.</div>}</div>
      </article>
    </section>
