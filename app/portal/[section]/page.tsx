...
export default async function PortalSection({params,searchParams}:{params:Promise<{section:string}>;searchParams:Promise<{org?:string;campaign?:string}>}){
  ...
  if(section==="contacts") content=<AgencyContacts organizationId={d.organizationId}/>
    ...
    {d.pinSession&&<div className="agency-notice">Payment preferences can be saved here. Sign in with your owner/admin email account to submit a payout request.</div>}
    <PayoutRequestPanel organizationId={d.organizationId} campaignId={d.campaign?.id||""} available={Number(d.balance.available)} threshold={d.org.organization_type==="detroit_fire_station"?Math.max(100,Number(d.campaign?.min_payout_threshold||0)):Number(d.campaign?.min_payout_threshold||0)} canRequest={!d.pinSession&&(d.platform||["owner","admin"].includes(d.memberRole||""))} openRequest={d.payoutRequests.find((r:any)=>["requested","approved","processing"].includes(r.status))||null}/>
