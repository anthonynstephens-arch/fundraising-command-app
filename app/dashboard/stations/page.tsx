...
async function saveStation(form: FormData) {
  ...
  const values = { name, brand_name_short: house, brand_primary_color: color, organization_type: STATION_TYPE }
  const result = id
...
export default async function StationAdmin() {
  ...
  const rows: Array<{id:string;name:string;brand_name_short:string;logo_url:string;brand_primary_color:string;reporting_start_date:string|null;collections:Awaited<ReturnType<typeof stationCollections>>}> = await Promise.all((stations || []).map(async s => ({ ...s, collections: await stationCollections(db, s.id) })))
  ...
  return <><section className="fc-page-header"><div><div className="fc-kicker">DETROIT FIRE DEPARTMENT</div><h1>Station command</h1><p>Individual houses. Shared oversight. $100 minimum cash-out per collection.</p></div><Link className="fc-btn" href="/dashboard/payouts">Review payout requests →</Link></section>
    ...
    <section className="fc-card"><h2>Add a station</h2><p>Create the house first, then use Manage access to add lieutenant and station-manager email accounts as Admin or Owner. Viewers cannot cash out. Upload the station logo below after creating the station.</p><form action={saveStation} style={{display:'flex',gap:16,flexWrap:'wrap',alignItems:'end'}}><label>Station / company name<input name="name" required maxLength={120} placeholder="Engine 17" /></label><label>House name<input name="house" maxLength={80}/></label><label>Station color<input type="color" name="color" defaultValue="#cf4538" /></label><button className="fc-btn fc-btn-primary">Create station</button></form></section>
    <section className="fc-list-grid">{rows.map(s => <article className="fc-card" key={s.id}><h2>{s.name}</h2><p>{s.collections.length} collections · {money(s.collections.reduce((a,c)=>a+Number(c.available),0))} available</p><div className="fc-head-actions"><Link className="fc-btn" href={'/station/'+s.id}>Open station</Link><Link className="fc-btn" href={'/dashboard/organizations/'+s.id+'/members'}>Manage access</Link><Link className="fc-btn" href={"/dashboard/stations/"+s.id+"/collections"}>Assign / sync collections</Link></div><OrganizationLogoUpload organizationId={s.id} name={s.name} logoUrl={s.logo_url} /><DepartmentReporting organizationId={s.id} startDate={s.reporting_start_date} /><details><summary>Edit station branding</summary><form action={saveStation} style={{display:'grid',gap:12,paddingTop:16}}><input name="id" type="hidden" value={s.id}/><label>Station name<input name="name" defaultValue={s.name} required/></label><label>House name<input name="house" defaultValue={s.brand_name_short || ''}/></label><label>Color<input type="color" name="color" defaultValue={s.brand_primary_color || '#cf4538'}/></label><button className="fc-btn">Save branding</button></form></details></article>)}</section>{!rows.length && <p>No Detroit Fire stations yet. Add your first station above.</p>}</>
}
