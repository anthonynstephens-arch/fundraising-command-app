-- One order per transaction. Stable Shopify IDs preserve payout_items foreign keys.
...
begin
  ...
  for line in select value from jsonb_array_elements(payload->'lines') loop
    ...
    -- Existing assignments are authoritative, including products removed from Shopify.
  ...
  insert into orders(organization_id,campaign_id,shopify_order_id,shopify_order_number,currency,subtotal,total,status,placed_at,fulfillment_status)
  values(target_org,first_campaign,regexp_replace(payload->>'id','^.*/',''),payload->>'name',payload->>'currencyCode',
  ...
  on conflict(shopify_order_id) do update set status=excluded.status,fulfillment_status=excluded.fulfillment_status,updated_at=now()
