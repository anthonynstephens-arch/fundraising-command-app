...
const defaults:Preferences={email_enabled:false,browser_enabled:false,in_app_enabled:true,new_sales:true,payout_updates:true,campaign_milestones:true,sync_issues:true}
export default function PortalNotificationSettings({organizationId,userEmail}:{organizationId:string;userEmail:string}){
 ...
 .then(async ([data,sub])=>{let connected=false;if(sub&&'Notification' in window&&Notification.permission==='granted'){const r=await fetch('/api/portal/push',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({organizationId,action:'status',subscription:sub.toJSON()})});if(!r.ok)throw Error();connected=(await r.json()).subscribed}if(live){setPreferences({...defaults,...data.preferences});setEnabled(connected);setLoaded(true)}}).catch(()=>{if(live)setMessage('Unable to load settings. Reload to try again.')})
 return()=>{live=false}
 },[organizationId])
 async function persist(next:Preferences){const r=await fetch('/api/portal/preferences',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({organizationId,...next})});if(!r.ok)throw Error('Could not save notification settings.');setPreferences(next);window.dispatchEvent(new Event('fc-notifications-changed'))}
 ...
 <details className="fc-email-status"><summary>Email delivery</summary><p>Email is not connected to this notification service yet. In-app and device alerts are available now. {userEmail.includes('@')?'Your signed-in email remains unchanged.':''}</p></details><InstallGuide/></section>
}
