...
function emitStoreEvent(name:string,detail:Record<string,unknown>){
...
function useCampaignCart(campaign:Campaign){
  const key=`fundraiser-command-cart:${campaign.id}`
  const [items,setItems]=useState<CartItem[]>([])
  const [ready,setReady]=useState(false)
  useEffect(()=>{
    try{const saved=localStorage.getItem(key);if(saved){const parsed=JSON.parse(saved);if(Array.isArray(parsed))setItems(parsed)}}catch{}
    setReady(true)
  },[key])
  useEffect(()=>{if(ready)localStorage.setItem(key,JSON.stringify(items))},[items,key,ready])
...
function campaignIsOpen(campaign:Campaign){
  const now=Date.now()
...
export function CampaignStorefront({campaign}:{campaign:Campaign}){
  ...
  return <main className="store-page"><StoreHeader campaign={campaign} count={cart.count} onCart={()=>setCartOpen(true)}/><CampaignIntro campaign={campaign}/>
    ...
    </section><footer className="store-footer"><b>Fundraiser Command</b><span>Secure checkout powered by Shopify</span></footer><CartDrawer campaign={campaign} cart={cart} open={cartOpen} onClose={()=>setCartOpen(false)}/></main>
}

function findVariant(product:StoreProduct,selected:Record<string,string>){
